...
def _numbers_close(a, b, rel=0.02, abs_tol=1.0):
    try:
        a = float(a); b = float(b)
...
def validate_ai_explanation(explanation, evidence):
    """Validate only claims that can be checked from the supplied event evidence.

    Returns exactly four high-level checks. REVIEW means the available evidence
    is insufficient to verify a claim; it is not treated as an error.
    """
    text = json.dumps(explanation, default=str, ensure_ascii=False)
    text_low = text.lower()
    event = evidence.get("event", {})
    stats = evidence.get("event_observations", {}).get("statistics", {})
    baseline = evidence.get("healthy_baseline", {}).get("reference", {})

    results = []

    # 1) Event time & duration
    expected_start = str(event.get("start_time", ""))[:16]
    expected_end = str(event.get("end_time", ""))[:16]
    expected_duration = event.get("duration_min")
    duration_claims = re.findall(r"(?:about|roughly|approximately|~)?\s*(\d+(?:\.\d+)?)\s*(?:minutes?|mins?|min)\b", text_low)
    duration_ok = True
    if expected_duration is not None and duration_claims:
        duration_ok = any(_numbers_close(x, expected_duration, rel=0.10, abs_tol=1.0) for x in duration_claims)
    if expected_start and expected_start not in text:
        # The exact event metadata is injected by Python; this is only a warning
        # if the LLM text itself does not repeat it.
        duration_detail = f"Selected event is {expected_start} → {expected_end}, {expected_duration:g} min."
    else:
        duration_detail = f"Selected event time and duration match: {expected_start} → {expected_end}, {expected_duration:g} min."
    results.append({
        "name": "Event time & duration",
        "status": "PASS" if duration_ok else "FAIL",
        "detail": duration_detail if duration_ok else "A duration stated in the explanation does not match the selected event."
    })
    ...
    direction_patterns = [
        ("dc_power_kw", r"(?:dc\s+power|dc power).*?(?:decreased|dropped|fell|reduced|declined)", "decreased"),
        ("dc_power_kw", r"(?:dc\s+power|dc power).*?(?:increased|rose|grew|increased)", "increased"),
        ("ac_power_kw", r"(?:ac\s+power|ac power).*?(?:decreased|dropped|fell|reduced|declined)", "decreased"),
        ("ac_power_kw", r"(?:ac\s+power|ac power).*?(?:increased|rose|grew|increased)", "increased"),
    ]
    for col, pattern, claimed in direction_patterns:
        if re.search(pattern, text_low, flags=re.S):
            direction_found = True
            item = stats.get(col, {})
            start_v, end_v = item.get("start"), item.get("end")
            if start_v is None or end_v is None:
                direction_errors.append(f"{col} direction could not be checked")
                continue
            actual = "decreased" if float(end_v) < float(start_v) else "increased" if float(end_v) > float(start_v) else "stable"
            if actual != claimed:
                direction_errors.append(f"{col.replace('_kw','').upper()} was actually {actual}")
    if direction_errors:
        results.append({"name":"Power change direction","status":"FAIL","detail":"; ".join(direction_errors)+"."})
    else:
        detail = "AC/DC power direction matches the event data." if direction_found else "No AC/DC power direction claim was made."
        results.append({"name":"Power change direction","status":"PASS","detail":detail})

    # 3) Numerical values
    # Ignore temperature numbers here; temperature/threshold is checked separately.
    unit_claims = re.findall(r"(?<!\d)(\d+(?:\.\d+)?)\s*(kW|kw|A|a|%)\b", text)
    known = []
    for col, item in stats.items():
        ...
        for key in ("start", "end", "min", "max", "mean", "median"):
            if item.get(key) is not None:
                known.append(float(item[key]))
        if col in ("dc_power_kw", "ac_power_kw", "dc_current_a", "ac_current_a") and item.get("start") is not None and item.get("end") is not None:
            known.append(abs(float(item["end"]) - float(item["start"])))
    # Add feature-contribution percentages when present.
    for item in evidence.get("feature_contributions", []) or []:
        if item.get("mean_contribution_pct") is not None:
            known.append(float(item["mean_contribution_pct"]))

    unmatched = []
    for value, unit in unit_claims:
        # The event-duration and date metadata are handled by their own check.
        if unit.lower() == "a" and float(value) == 0:
            continue
        if not any(_numbers_close(value, candidate, rel=0.02, abs_tol=1.0) for candidate in known):
            unmatched.append(f"{value:g} {unit}")
    if unmatched:
        results.append({"name":"Numerical values","status":"FAIL","detail":"Unmatched numeric claims: " + ", ".join(unmatched[:4]) + "."})
    else:
        results.append({"name":"Numerical values","status":"PASS","detail":"Reported measurement values are consistent with supplied event evidence."})

    # 4) Temperature / threshold claim
    temp_claimed = bool(re.search(r"temperature|temp\b|°c|\bc\b", text_low))
    temp = stats.get("inverter_temperature_c", {})
    actual_temp = temp.get("end", temp.get("mean"))
    temp_ref = baseline.get("inverter_temperature_c", {}) if isinstance(baseline, dict) else {}
    baseline_high = temp_ref.get("typical_high_q90")
    baseline_median = temp_ref.get("median")
    threshold_nums = [float(x) for x in re.findall(r"(?:limit|threshold|upper healthy|healthy limit)[^\d]{0,20}(\d+(?:\.\d+)?)\s*°?c", text_low)]

    if not temp_claimed:
        results.append({"name":"Temperature / threshold claim","status":"PASS","detail":"No temperature or threshold claim was made."})
    elif actual_temp is None:
        results.append({"name":"Temperature / threshold claim","status":"REVIEW","detail":"Temperature is not available in the supplied event evidence."})
    elif threshold_nums and baseline_high is not None:
        threshold = threshold_nums[0]
        if not _numbers_close(threshold, baseline_high, rel=0.02, abs_tol=1.0):
            results.append({"name":"Temperature / threshold claim","status":"FAIL","detail":f"The stated {threshold:g}°C limit does not match the supplied healthy reference."})
        elif float(actual_temp) < threshold:
            results.append({"name":"Temperature / threshold claim","status":"PASS","detail":f"Temperature {float(actual_temp):.1f}°C is below the supplied {threshold:g}°C healthy reference."})
        else:
            results.append({"name":"Temperature / threshold claim","status":"FAIL","detail":f"Temperature {float(actual_temp):.1f}°C is not below the supplied {threshold:g}°C healthy reference."})
    elif threshold_nums:
        results.append({"name":"Temperature / threshold claim","status":"REVIEW","detail":"A temperature limit was stated, but no independently supplied healthy threshold is available to verify it."})
    elif baseline_high is not None:
        if "high" in text_low:
            high_ok = baseline_median is None or float(actual_temp) >= float(baseline_median)
            if high_ok:
                results.append({"name":"Temperature / threshold claim","status":"PASS","detail":f"Temperature {float(actual_temp):.1f}°C is supported by the available healthy baseline."})
            else:
                results.append({"name":"Temperature / threshold claim","status":"FAIL","detail":"The explanation calls temperature high, but the supplied baseline does not support that wording."})
        else:
            results.append({"name":"Temperature / threshold claim","status":"PASS","detail":"Temperature value is available and the available baseline can be checked."})
    else:
        results.append({"name":"Temperature / threshold claim","status":"REVIEW","detail":"Temperature is available, but no independently supplied healthy threshold is available to verify the threshold claim."})
...
if cached is None:
    ...
    # --------------------------------------------------------
    # AI EXPLANATION VALIDATION
    # Only four evidence-based checks are shown to the user.
    # --------------------------------------------------------
    validation_results = validate_ai_explanation(
        cached["explanation"],
        cached["evidence"],
    )

    pass_count = sum(r["status"] == "PASS" for r in validation_results)
    review_count = sum(r["status"] == "REVIEW" for r in validation_results)
    fail_count = sum(r["status"] == "FAIL" for r in validation_results)

    if fail_count:
        validation_title = f"{fail_count} check{'s' if fail_count != 1 else ''} need attention"
        validation_badge = "#FDECEC"
        validation_text = "#B42318"
    elif review_count:
        validation_title = f"{review_count} check{'s' if review_count != 1 else ''} need review"
        validation_badge = "#FFF4D6"
        validation_text = "#8A5A00"
    else:
        validation_title = "All checks passed"
        validation_badge = "#E8F7EE"
        validation_text = "#087443"

    st.markdown(
        f'''<div style="background:#FFFFFF;border:1px solid #D9E1EA;border-radius:12px;padding:14px 18px;margin-top:14px;">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
                <div>
                    <div style="font-size:1.15rem;font-weight:700;color:#0F172A;">Validation</div>
                    <div style="font-size:0.82rem;color:#64748B;margin-top:3px;">Checks whether the explanation is consistent with the selected event's data and supplied evidence.</div>
                </div>
                <div style="background:{validation_badge};color:{validation_text};border:1px solid #D9E1EA;border-radius:999px;padding:5px 10px;font-size:0.78rem;font-weight:600;white-space:nowrap;">{validation_title}</div>
            </div>
        </div>''',
        unsafe_allow_html=True,
    )
    ...
    for item in validation_results:
        ...
        bg = "#DDF7E8" if status == "PASS" else "#FFF0C2" if status == "REVIEW" else "#FDE2E2"
        fg = "#087443" if status == "PASS" else "#8A5A00" if status == "REVIEW" else "#B42318"
        st.markdown(
            f'''<div style="display:flex;gap:10px;align-items:flex-start;padding:9px 4px;border-bottom:1px solid #EEF2F6;">
                <span style="width:20px;height:20px;border-radius:50%;background:{bg};color:{fg};display:inline-flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex:0 0 20px;">{icon}</span>
                <div><div style="font-size:0.9rem;font-weight:600;color:#1E293B;">{html.escape(item['name'])}</div>
                <div style="font-size:0.76rem;color:#64748B;margin-top:2px;line-height:1.45;">{html.escape(item['detail'])}</div></div>
            </div>''',
            unsafe_allow_html=True,
        )

    st.markdown(
        '<div style="background:#F7F9FC;border:1px solid #D9E1EA;border-radius:8px;padding:8px 10px;margin-top:8px;font-size:0.74rem;color:#64748B;">Validation checks evidence consistency. A passed explanation does not confirm the physical root cause.</div>',
        unsafe_allow_html=True,
    )
